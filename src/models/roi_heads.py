"""
Script adpated from https://github.com/pytorch/vision/tree/main/torchvision/models/detection
"""
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, scores):
    ...
    weights = torch.ones_like(labels, device=labels.device).float()
    weights[labels==1] = 14.22
    weights[labels==2] = 1.27
    weights[labels==3] = 17.60
    weights[labels==4] = 25.15
    weights[labels==5] = 31.79
    weights[labels==6] = 77.21
    classification_loss = torch.sum(weights/torch.sum(weights)*scores*F.cross_entropy(class_logits, labels, reduction='none'))
...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, gt_scores):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image, gt_scores_in_image in zip(proposals, gt_boxes, gt_labels, gt_scores):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                _, indices = match_quality_matrix.max(dim=0)
                ...
                # discard indices that are between tau_m and tau_h
                label_per_indices = gt_labels_in_image[indices]
                label_to_discard = label_per_indices == -2
                labels_in_image[label_to_discard] = -1.
    ...
    def select_training_samples(self,
        ...
        for i, t in enumerate(targets):
            if 'scores' in t.keys():
                gt_scores.append(t["scores"])
            else:
                gt_scores.append(torch.ones_like(gt_labels[i]))
    ...
    def forward(self,
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg = fastrcnn_loss(
                class_logits, box_regression, labels, regression_targets, scores)
            losses = {
